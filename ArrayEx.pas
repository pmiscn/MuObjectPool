...
type
  ...
  private
    ...
    class function CompareT2(const A, B: T; const Comparer: IComparer<T>): Boolean; static;
    class procedure CopyArray(var FromArray, ToArray: TArray<T>; FromIndex: NativeInt = 0;
      ToIndex: NativeInt = 0; Count: NativeInt = -1); static;
    class procedure MoveArray(var AArray: array of T; FromIndex, ToIndex, Count: Integer); static;
  ...
  public
    ...
    procedure Unique();
    procedure Unique2(const Comparer: IComparer<T>);
    // 排序
    procedure Sort(); overload;
    procedure Sort(const Comparer: IComparer<T>); overload;
    procedure Sort(const Comparer: IComparer<T>; Index, Count: Integer); overload;
    // 搜索
    function BinarySearch(const Item: T; out FoundIndex: Integer; const Comparer: IComparer<T>;
    ...
    function BinarySearch(const Item: T): Boolean; overload;
    // 简单搜索是否存在
    function FindByLoop(const Item: T): Boolean;
...
function TArrayEx<T>.BinarySearch(const Item: T; out FoundIndex: Integer): Boolean;
begin
  Result := BinarySearch(Item, FoundIndex, TComparer<T>.Default, Low(FData), Length(FData));
end;

function TArrayEx<T>.BinarySearch(const Item: T): Boolean;
var
  FoundIndex: Integer;
begin
  Result := BinarySearch(Item, FoundIndex);
end;
...
class procedure TArrayEx<T>.MoveArray(var AArray: array of T; FromIndex, ToIndex, Count: Integer);
var
  i: Integer;
begin
  if Count > 0 then
  begin
    ...
    // AArray[ToIndex + i] := AArray[FromIndex + i];
    // 从尾数填坑开始
    if FromIndex < ToIndex then   //新增数据
    begin
      //新的尾号 = 旧的的尾号
      for i := Count - 1 downto 0 do
      begin
        AArray[ToIndex - (Count - 1 - i)] := AArray[FromIndex + i] //
      end;
    end
    else if FromIndex > ToIndex then   //缩减数组
    begin
      //起始位置左移
      for i := Count - 1 downto 0 do
      begin
        AArray[ToIndex+ Count-1-i] := AArray[FromIndex+Count-1-i] //
      end;
    end;
  end;
end;
...
class function TArrayEx<T>.CompareT2(const A, B: T; const Comparer: IComparer<T>): Boolean;
begin
...
function TArrayEx<T>.FindByLoop(const Item: T): Boolean;
var
  Item2: T;
begin
  Result := False;
  for Item2 in FData do
  begin
    Result := CompareT(Item, Item2);
    if Result then
      Break;
  end;
end;
...
function TArrayEx<T>.Insert(AIndex: NativeInt; const Values: array of T): NativeInt;
var
  i: Integer;
begin
  SetLength(FData, Length(Values) + Length(FData));
  MoveArray(FData, AIndex, AIndex + Length(Values) - 1, Length(FData) - AIndex);
  // TINTIN       //原来最后一个参数为Length(Values)
...
function TArrayEx<T>.Insert(AIndex: NativeInt; const Values: TArray<T>): NativeInt;
var
  i: Integer;
begin
  SetLength(FData, Length(FData) + Length(Values));
  MoveArray(FData, AIndex, AIndex + Length(Values), Length(FData) - Length(Values)); // AIndex
  for i := 0 to Length(Values) - 1 do
    FData[AIndex + i] := Values[i];
  Result := AIndex;
end;

function TArrayEx<T>.Insert(AIndex: NativeInt; const Values: TArrayEx<T>): NativeInt;
begin
...
procedure TArrayEx<T>.Unique2(const Comparer: IComparer<T>);
var
  i, J: Integer;
  Tmp: TArrayEx<T>;
  Flag: Boolean;
begin
  ...
  begin
    ...
    for J := High(Tmp.FData) downto Low(Tmp.FData) do
    begin
      if CompareT2(FData[i], Tmp[J], Comparer) then
      begin
        Flag := True;
        Break;
      end;
