...
type
  ...
  // of object;
  TOnProgress = reference to procedure(ATotal, AProgress: int64;
    var Cancel: boolean);
  TOnStatus = reference to procedure(AStatus: String);

  TIconSize = (isSmall, isLarge, isOpen);
  TObjAttr = (oaCanCopy, oaCanDelete, oaCanLink, oaCanMove, oaCanRename,
    oaDropTarget, oaHasPropSheet, oaIsLink, oaIsReadOnly, oaIsShare,
    oaHasSubFolder, oaFileSys, oaFileSysAnc, oaIsFolder, oaRemovable,
    oaEncrypted);

  TObjAttributes = record
    CanCopy, CanDelete, CanLink, CanMove, CanRename, DropTarget, HasPropSheet,
      isStorage, IsLink, IsReadOnly, IsShare, IsHidden, HasSubFolder, FileSys,
      FileSysAnc, IsFolder, Removable, Encrypted: boolean;
  end;

  TTypeExe = (teWin32, teWin64, teDOS, teWin16, tePIF, tePOSIX, teOS2,
    teUnknown, teError);
  TSubType = (stUnknown, stApp, stDLL, stSLL, stDrvUnknown, stDrvComm,
    stDrvPrint, stDrvKeyb, stDrvLang, stDrvDisplay, stDrvMouse, stDrvNetwork,
    stDrvSystem, stDrvInstall, stDrvSound, stFntUnknown, stFntRaster,
    stFntVector, stFntTrueType, stVXD);
  ...
  TFileVersionInfo = record
    FileType, CompanyName, FileDescription, FileVersion, InternalName,
      ...
      ProductVersion, Comments, SpecialBuildStr, PrivateBuildStr: String;
  ...
  TDocInfo = record
    Title, Subject, Author, Keywords, Comments, Template, LastAuthor,
      RevNumber: String;
    EditTime: integer;
  ...
  public
    class function FromStream(astream: TStream; AProgress: TOnProgress = nil)
      : string; static;
    class function FromFile(aFileName: String; AProgress: TOnProgress = nil)
      : string; static;
  ...
  public
    class function create(aFileName: string; AProgress: TOnProgress = nil;
      AFileProgress: TOnProgress = nil; AStatus: TOnStatus = nil)
      : TLocalFileInfo; static;
    class function GetFileMD5(aFileName: string; AProgress: TOnProgress = nil)
      : String; static;
    class function GetFileTimes(aFileName: string;
      var Created, Accessed, Modified: TDateTime): boolean; static;

    class function GetFileCreateTimes(aFileName: string): TDateTime; static;

    class function SetDateTime(aFileName: string; const DateTime: TDateTime)
      : integer; static;
    class function GetFileAttr(aFileName: string): integer; static;
    class function SetFileAttr(aFileName: string; Attr: integer)
      : integer; static;
    ...
    class function GetFileIcon(aFileName: string; isSize: TIconSize)
      : hIcon; static;
    class function GetFileIconByExt(aExt: string; isSize: TIconSize)
      : hIcon; static;
    class function GetFileOwner(aFileName: string; var Domain, Username: String)
      : boolean; static;
...
{$ENDIF}
    ...
    class function GetFileVersionInfo(aFileName: string)
      : TFileVersionInfo; static;
    class function GetFileVersionNumber(aFileName: string = ''): String; static;
    class function GetObjAttr(aFileName: string; oaAttr: TObjAttr)
      : boolean; static;
    class function GetObjAttributes(aFileName: string;
      var oaAttr: TObjAttributes): boolean; static;
    class function IsInUse(aFileName: string): boolean; static;
    class function IsAscii(aFileName: string): boolean; static;

    class function IfThen<T>(aValue: boolean; const ATrue, AFalse: T)
      : T; static;
...
procedure FileFind(StartingDirectory, Filestyle: string; FilesFound: TStrings;
  aHasFolder: boolean = false; ahassub: boolean = true);
...
class function TLocalFileInfo.IfThen<T>(aValue: boolean;
  const ATrue, AFalse: T): T;
begin
  if aValue then
    result := ATrue
  else
...
begin
  result := 0;
  if FileTimeToLocalFileTime(FileTime, LocalFileTime) and
    FileTimeToDosDateTime(LocalFileTime, LongRec(FileDate).Hi,
...
class function TLocalFileInfo.create(aFileName: string;
  AProgress: TOnProgress = nil; AFileProgress: TOnProgress = nil;
  AStatus: TOnStatus = nil): TLocalFileInfo;
var
  st: Tstringlist;
  I, C: integer;
  isCancel: boolean;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        // AStatus(st[I]);
        result.Files[I] := TLocalFileInfo.create(st[I], AProgress,
          AFileProgress, AStatus);
      end;
    finally
      st.Free;
    end;
  ...
  begin
    ...
    // result.Version := TLocalFileInfo.GetFileVersionInfo(aFileName).FileVersion;
    result.Version := TLocalFileInfo.GetFileVersionNumber(aFileName);

    TLocalFileInfo.GetFileTimes(aFileName, result.CreateTime, result.AccessTime,
      result.ModifyTime);
  end;
end;
...
class procedure TLocalFileInfo.GetDLLInfo(aFileName: string;
  aStrings: TStrings);
type
...
begin
  ...
  begin
    h := CreateFile(PWideChar(DllName), GENERIC_READ, FILE_SHARE_READ or
      FILE_SHARE_WRITE, nil, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    if h <> INVALID_HANDLE_VALUE then
      try
        ImageDebugInformation := MapDebugInformation(h,
          PAnsiChar(DllName), nil, 0);
...
class function TLocalFileInfo.GetFileIcon(aFileName: string;
  isSize: TIconSize): hIcon;
var
...
begin
  ...
  SHGetFileInfo(PWideChar(aFileName), dwFileAttr, shFileInfo,
    cbFileInfo, uFlags);
  result := shFileInfo.hIcon;
end;

class function TLocalFileInfo.GetFileIconByExt(aExt: string;
  isSize: TIconSize): hIcon;
var
  shFileInfo: TSHFileInfo;
  dwFileAttr, cbFileInfo, uFlags: uint;
...
class function TLocalFileInfo.GetFileVersionInfo(aFileName: string)
  : TFileVersionInfo;
var
  rSHFI: TSHFileInfo;
  iRet: integer;
  ...
  begin
    ...
    if Winapi.Windows.GetFileVersionInfo(PWideChar(sAppNamePath), VerHandle,
      VerSize, VerBuf) and
...
begin
  ...
  // Get the file type
  if SHGetFileInfo(PWideChar(sAppNamePath), 0, rSHFI, sizeof(rSHFI),
    SHGFI_TYPENAME) <> 0 then
  begin
    result.FileType := rSHFI.szTypeName;
  end;

  iRet := SHGetFileInfo(PWideChar(sAppNamePath), 0, rSHFI, sizeof(rSHFI),
    SHGFI_EXETYPE);
  if iRet <> 0 then
  begin
...
class function TLocalFileInfo.SetFileAttr(aFileName: string;
  Attr: integer): integer;
begin
  result := FileSetAttr(aFileName, Attr);
end;

class function TLocalFileInfo.GetFileMD5(aFileName: string;
  AProgress: TOnProgress = nil): String;
begin
...
class function TLocalFileInfo.GetFileOwner(aFileName: string;
...
begin
  ...
  try
    if not GetFileSecurity(PWideChar(aFileName), OWNER_SECURITY_INFORMATION,
...
class function TLocalFileInfo.GetFileTimes(aFileName: string;
  var Created, Accessed, Modified: TDateTime): boolean;
var
  h: THandle;
...
class function TLocalFileInfo.GetObjAttributes(aFileName: string;
  var oaAttr: TObjAttributes): boolean;
var
  sfgao: integer;
var
  shFileInfo: TSHFileInfo;
...
begin
  sfgao := 0;
  uFlags := ATTR_ALL;
  result := SHGetFileInfo(PWideChar(aFileName), dwFileAttr, shFileInfo,
    cbFileInfo, uFlags) <> 0;
  if result then
    with oaAttr do
    begin
...
class function TLocalFileInfo.GetObjAttr(aFileName: string;
  oaAttr: TObjAttr): boolean;
var
  sfgao: integer;
var
  shFileInfo: TSHFileInfo;
...
begin
  ...
  if not FileExists(aFileName) then
    exit;
  HFileRes := CreateFile(PWideChar(aFileName), GENERIC_READ or GENERIC_WRITE, 0,
...
class function TLocalFileInfo.SetDateTime(aFileName: string;
  const DateTime: TDateTime): integer;
var
  FileHandle: integer;
  Succes: integer;
begin
...
class function TMd5Str.FromStream(astream: TStream;
  AProgress: TOnProgress = nil): string;
var
  MD5: THashMD5;
  buffer: Pointer;
...
begin
  ...
  BufSize := TLocalFileInfo.IfThen<LongInt>(Total < MD5BufSize, Total,
    MD5BufSize);
...
class function TMd5Str.FromFile(aFileName: String;
  AProgress: TOnProgress = nil): string;
...
begin
  ...
  FileHandle := CreateFile(pchar(aFileName), GENERIC_READ, FILE_SHARE_READ or
    ...
    begin
      try
        ...
        begin
          MD5 := THashMD5.create;
          try
            MD5.Update(ViewPointer,
              Winapi.Windows.GetFileSize(FileHandle, nil));
            result := MD5.HashAsString;
          finally
